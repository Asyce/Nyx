...
impl<W: Write> StatusWriter<W> {
    ...
    pub fn state(&mut self, state: State) -> io::Result<()> {
        self.write(state, None, None, None)
    }

    pub fn failed(&mut self, code: ErrorCode) -> io::Result<()> {
        self.write(State::Failed, None, Some(code), None)
    }

    pub fn exported(&mut self, item_count: usize, output_file: &str) -> io::Result<()> {
        self.write(State::Exported, Some(item_count), None, Some(output_file))
    }
...
mod tests {
    ...
    fn job_derived_event_cancels_without_console_input() {
        let name = format!("{EVENT_PREFIX}cancel-test_01")
            .encode_utf16()
            .chain([0])
            .collect::<Vec<_>>();
        let owner = unsafe { CreateEventW(None, true, false, PCWSTR(name.as_ptr())) }.unwrap();
