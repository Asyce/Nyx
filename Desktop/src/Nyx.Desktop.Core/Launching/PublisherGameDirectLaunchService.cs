...
public sealed class PublisherGameDirectLaunchService
{
    ...
    public PublisherGameDirectLaunchResult CheckGame(string gameId, string? root)
    {
        var profile = GetProfile(gameId);
        try
        {
            using var inspection = validator.InspectProtected(gameId, root);
            return EvaluateProtected(gameId, root, profile, inspection);
        }
    ...
    public PublisherGameDirectLaunchResult LaunchGame(string gameId, string? root)
    {
        var profile = GetProfile(gameId);
        var initial = CheckGame(gameId, root);
        ...
        try
        {
            ...
            var fresh = EvaluateProtected(gameId, root, profile, freshInspection);
            ...
            catch (Exception exception) when (IsBoundaryFailure(exception))
            {
                ...
                return LaunchElevated(gameId, root, profile, fresh.Specification);
            }
        }
        catch (Exception exception) when (IsBoundaryFailure(exception))
        {
    ...
    {
        using var inspection = validator.InspectProtected(gameId, root);
        var fresh = EvaluateProtected(gameId, root, profile, inspection);
        if (fresh.Status is not PublisherGameLaunchStatus.Ready
            || fresh.Specification is null
