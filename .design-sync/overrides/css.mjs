// forked from design-sync lib/css.mjs — inline the app CSS's site-served image
// assets (backgrounds, masks, frames) as data URIs; they live at site-relative
// paths that don't exist in the design environment. Oversized originals are
// substituted by compressed copies in .design-sync/assets/ (same basename).
...
// Fork addition: rewrite the app CSS's relative image url()s to data URIs.
// Resolution order: .design-sync/assets/<basename> (compressed stand-in for
// oversized originals) → the path relative to the CSS's source dir. Files that
// can't be found or exceed the cap are left as-is (a 404'd decoration is
// harmless; a megabyte of base64 in every design render is not).
const IMG_SRC_DIR = resolve(process.cwd(), 'Site/src/styles');
const IMG_SUBST_DIR = resolve(process.cwd(), '.design-sync/assets');
const IMG_INLINE_CAP = 350 * 1024;

function inlineBundleImages(css) {
  let inlined = 0, skipped = 0;
  const done = new Map();
  for (const m of [...css.matchAll(/url\(\s*['"]?([^'")]+)['"]?\s*\)/gi)]) {
    const u = m[1];
    if (/^(?:https?:|data:|\.\/fonts\/|#)/.test(u) || done.has(u)) continue;
    if (/\.(ttf|otf|woff2?|eot)(?:[?#]|$)/i.test(u)) continue;
    const name = basename(u.split(/[?#]/)[0]);
    const cand = [join(IMG_SUBST_DIR, name), resolve(IMG_SRC_DIR, u)].find((f) => existsSync(f));
    if (!cand) { done.set(u, null); skipped++; continue; }
    const buf = readFileSync(cand);
    if (buf.length > IMG_INLINE_CAP) { done.set(u, null); skipped++; continue; }
    const mime = buf[0] === 0xff && buf[1] === 0xd8 ? 'image/jpeg'
      ...
      : 'image/png';
    done.set(u, `data:${mime};base64,${buf.toString('base64')}`);
    inlined++;
  }
  for (const [u, dataUri] of done) if (dataUri) css = css.split(u).join(dataUri);
  console.error(`  _ds_bundle.css images (fork): ${inlined} url(s) inlined as data URIs${skipped ? `, ${skipped} left as-is (missing or >${IMG_INLINE_CAP / 1024}KB)` : ''}`);
  return css;
}
...
export function rewriteBundleFontFaces({ out, bundleCss }) {
  ...
  if (!/@font-face/i.test(css)) { writeFileSync(p, inlineBundleImages(css)); return; }
  ...
  writeFileSync(p, inlineBundleImages(next));
}
