...
- 6bcb877 — **HSR traces (3.7) DONE**: generator decomposes `skill_trees` point01-04 into per-level `talentStages` (Basic ATK max 6, Skill/Ult/Talent max 10) + `talentBase`/`talentBaseCost` (minor traces); UI has a "Max" toggle (default) + 4 typeable Basic/Skill/Ultimate/Talent inputs (CM_TALENT_CFG). Verified stages+base sum exactly to the all-to-max total.
- **4.1 (stale chars) — DATA-SOURCE LIMIT.** Himeko Nova + Gilgamesh exist in Prydwen but NOT in Nanoka (Fate collab / custom), so they have no material data and fall back to synthetic mats. A Nanoka HSR re-scrape was attempted (`--game hsr --skip-assets`) and did NOT add them (confirmed not on nanoka.cc); it was reverted (stashed) because it changed 889 files for no gain and pulled a `beta/` folder (now gitignored). To truly fix these two, they must be added to Nanoka or given manual material data.
- **3.2b (wiki obtaining) — addressed as feasible.** Display fixed (3.2a), junk cleaned (recommendation/Go to collect), GI talent books → "[Region] Talent Domain" (3.3). Remaining wrong-source materials need specific examples to backfill.
