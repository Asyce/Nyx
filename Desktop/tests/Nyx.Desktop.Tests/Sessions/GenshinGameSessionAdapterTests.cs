...
public sealed class GenshinGameSessionAdapterTests
{
    ...
    [Fact]
    public async Task Fail_closed_rows_have_no_observation_or_launch_capability()
    {
        foreach (var gameId in new[] { "hsr", "zzz", "wuwa", "ae" })
        {
            var adapter = new FailClosedGameSessionAdapter(gameId);
        ...
        Assert.Throws<ArgumentException>(() => new FailClosedGameSessionAdapter("gi"));
        Assert.Throws<UnsupportedGameException>(() => new FailClosedGameSessionAdapter("genshin"));
    }
