use crate::capture::{FrameEvent, FrameSource};
...
pub const FILTER: &str = "udp and (port 22101 or port 22102)";
...
fn configure_capture<A: CaptureSetupApi + ?Sized>(
    ...
    let filter = CString::new(FILTER).expect("the fixed filter has no NUL byte");
...
impl FrameSource for NpcapFrameSource {
    fn start(&mut self) -> Result<(), String> {
        ...
        self.capture = Some(configure_capture(api, &self.device)?);
        self.started = true;
        Ok(())
    }
...
mod tests {
    ...
    fn every_setup_failure_closes_and_frees_exactly_once() {
        ...
        for (failure, expected_close, expected_free) in [
            ...
            assert!(configure_capture(&api, &device).is_err(), "{failure:?}");
        ...
        let handle = configure_capture(&api, &device).unwrap();
    ...
    fn capture_contract_is_narrow_and_bounded() {
        assert_eq!(FILTER, "udp and (port 22101 or port 22102)");
    ...
    #[test]
    fn actual_compiled_filter_accepts_only_genshin_udp_frames() {
        ...
        for frame in [
            ipv4_packet(17, 22101, 443),
            ipv4_packet(17, 443, 22102),
            ipv6_udp(22102, 443),
            ipv6_udp(443, 22101),
        ] {
        ...
        let destination_match = ipv4_packet(17, 443, 22102);
        assert!(!offline_matches(
            &api,
            &filter_guard.program,
            &destination_match,
