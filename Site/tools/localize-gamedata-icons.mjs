// Downloads any GameData CDN icons referenced by the banner data into the local
...
//   local   Database/GameData/<game>/assets/<rest>
//
...
const REMOTE_RE = /https:\/\/static\.gamedata\.cc\/assets\/([^/]+)\/([^"\\]+?\.(?:webp|png|jpe?g))/gi;
...
const entries = [...collectUrls().entries()].map(([url, { game, rest }]) => ({
  url,
  dest: path.resolve(dbDir, 'GameData', game, 'assets', rest),
}));

if (!entries.length) {
  console.log('No GameData CDN icons referenced — nothing to localize.');
  process.exit(0);
}
...
console.log(`Localized GameData icons: ${count('downloaded')} downloaded, ${count('cached')} cached, ${count('missing')} missing (of ${entries.length}).`);
if (count('missing')) {
